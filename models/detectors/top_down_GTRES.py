...
class TopDownGTRES(BasePose):
    ...
    def __init__(self,
                 ...
                 neck=None,
                 keypoint_head=None,
                 keypoint_head_l=None,
                 keypoint_head_2conv=None,
        ...
        if keypoint_head is not None:
            ...
            self.keypoint_head_l =  builder.build_head(keypoint_head_l)
            self.keypoint_head_2conv = builder.build_head(keypoint_head_2conv)
    ...
    def init_weights_l(self, pretrained=None):
        ...
        load_state_dict(self.keypoint_head_l, ckpt_k)

        self.backbone_l.eval()
        self.keypoint_head_l.eval()
    ...
    def forward_train(self, img, target, target_weight, img_metas, **kwargs):
        ...
        if self.with_keypoint:
            ...
            output_2conv = self.keypoint_head_2conv(origin_out[2])
            with torch.no_grad():
                output_l = self.keypoint_head_l(output_l[3])
        ...
        if self.with_keypoint:
            ...
            keypoint_losses['s3tgt_loss'] = self.keypoint_head.get_loss(output_2conv, target, target_weight) * 0.5
            keypoint_losses['s3ts4_loss'] = self.keypoint_head_2conv.get_loss(output_2conv, output_l, target_weight) * 0.5
            keypoint_losses['r50thr_loss'] = self.keypoint_head.get_loss(output, output_h, target_weight) * 0.5
            ...
            keypoint_accuracy['r50t152_accuracy'] = self.keypoint_head.get_accuracy(output, output_l, target_weight)
